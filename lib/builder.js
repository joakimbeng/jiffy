...
function buildTagPages (tags, mainData) {
  return q.all(tags.map(function (tag) {
    var tagData = assign({path: '../', currentUrl: tag.tag.url, inTag: true}, mainData, tag);
...
function buildCategoryPages (categories, mainData) {
  return q.all(categories.map(function (category) {
    var categoryData = assign({path: '../', currentUrl: category.category.url, inCategory: true}, mainData, category);
...
function buildPostPages (posts, mainData) {
  return q.all(posts.map(function (post) {
    var postData = assign({post: post, path: '../', currentUrl: post.url, inPost: true}, mainData);
...
function buildIndexPage (mainData) {
  var indexData = assign({inIndex: true}, mainData);

  return build(indexData, 'index', 'index.html');
}

function buildFeed (mainData) {
  var feedData = assign({inFeed: true, lastBuildDate: mainData.buildTime.toGMTString()}, mainData);
